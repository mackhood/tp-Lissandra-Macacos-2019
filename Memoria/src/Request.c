	...
	if(segmento_buscado /*!= NULL*/){
		...
		//no se encuentra la pagina en memoria
			...
			if(mensaje_con_tabla->head == VALUE_FAILURE){
				...
				logInfo("[SELECT/REQ]: el filesystem posee la key");
...
bool insertReq (char* nombre_tabla, uint16_t key, char* value) {
	...
	//Chequeamos si el value pasado es mayor al tamanio maximo que puede tener un value (suma el \0)
	if((strlen(value)+1)>tamanio_value){
		logInfo("[INSERT/REQ]: El value solicitado es demasiado grande");
		pthread_mutex_unlock(&mutex_estructuras_memoria);
		return false;
	}
	else{
